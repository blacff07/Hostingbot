# -*- coding: utf-8 -*-
"""
Universal File Hosting Bot — by Blac (@NottBlac)
"""
...
def home():
    return """
    <html>
    <head><title>HostingBot — Blac</title></head>
    <body style="font-family:Arial;text-align:center;background:linear-gradient(135deg,#0f0c29,#302b63,#24243e);color:white;padding:50px;">
        <h1>HostingBot</h1>
        <p>by <b>@NottBlac</b> — 30+ file types — Running</p>
    </body>
    </html>
    """
...
shell_sessions = {}         # uid -> True when in shell session mode
exec_locks = {}             # key -> True when script is being launched
...
def clear_old_data():
    """Kill all processes and wipe all user files/scripts on restart"""
    ...
    for d in [UPLOAD_DIR, EXTRACT_DIR, PENDING_DIR]:
        if os.path.exists(d):
...
def is_running(uid, name):
    key = f"{uid}_{name}"
    if key not in scripts or not scripts[key].get('process'):
        return False
    try:
...
def update_user_info(msg):
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.execute('INSERT OR REPLACE INTO users (uid, name, username, last_seen) VALUES (?, ?, ?, ?)',
                     (uid, name.strip(), username, datetime.now().isoformat()))
        conn.commit()
        conn.close()
    except:
        pass
        ...
        if ext == '.py':
            ...
            for imp in imports:
                ...
                if mod in pkg_map and pkg_map[mod] and pkg_map[mod] not in installed and pkg_map[mod] not in new:
                    try:
                        res = subprocess.run(
                            [sys.executable, '-m', 'pip', 'install', '--quiet', pkg_map[mod]],
            ...
            for mod in requires:
                ...
                if base in node_map and node_map[base] not in installed and node_map[base] not in new:
                    try:
                        ...
                        if res.returncode == 0:
                            msgs.append(f"✅ {node_map[base]}")
                            new.add(node_map[base])
                        else:
                            msgs.append(f"❌ {node_map[base]}")
                    except:
                        msgs.append(f"⚠️ {node_map[base]}")
    except:
        pass
...
def handle_zip(zip_path, uid, extract_to, msg=None):
    try:
        ...
        if not main_file:
            return False, "No executable file found in ZIP"
...
LANG_MAP = {
    ...
    '.ts': ('TypeScript', '🔷'), '.bat': ('Batch', '🖥️'), '.ps1': ('PowerShell', '🔵')
}

def execute_script(uid, file_path, msg=None, work_dir=None):
    """Execute a script; prevents duplicate simultaneous launches."""
            ...
            if msg:
                try:
                    safe_edit(msg.chat.id, msg.message_id,
                             f"⚠️ `{name}` is already being started", 'Markdown')
                except:
                    pass
...
def _do_execute(uid, file_path, msg, work_dir, name, ext, key):
    ...
    if ext in STATIC_EXTS:
        if msg:
            ...
            url = f"{base_url}/file/{fhash}"
            mk = types.InlineKeyboardMarkup()
            mk.add(types.InlineKeyboardButton("🔗 View File", url=url))
    ...
    if ext not in LANG_MAP:
        if msg:
            safe_edit(msg.chat.id, msg.message_id, f"❌ Unsupported type: `{ext}`", 'Markdown')
    ...
    try:
        ...
        for attempt in range(1, 11):
            ...
            try:
                ...
                scripts[key] = {
                    'process': None, 'key': key, 'uid': uid, 'name': name,
                    'start': datetime.now(), 'log': log_path,
                ...
                scripts[key] = {
                    'process': p, 'key': key, 'uid': uid, 'name': name,
                    'start': datetime.now(), 'log': log_path,
        ...
        if msg:
            try:
                safe_edit(msg.chat.id, msg.message_id, f"❌ Error: `{str(e)[:200]}`", 'Markdown')
            except:
                pass
        return False, str(e)
    ...
    try:
        ...
        if len(result) > 4096:
            ...
            with open(tmp, 'rb') as f:
                bot.send_document(status.chat.id, f, caption=f"`$ {full_cmd}`", parse_mode='Markdown')
            os.remove(tmp)
...
def cmd_shell(message):
    ...
    if len(parts) > 1 and parts[1].strip():
        ...
        safe_reply(message, "💻 *Shell Active*\nSend commands directly\\. Supports multiple lines\\.", 'MarkdownV2', mk)
...
def cmd_start(message):
    ...
    role = "👑 Owner" if uid == OWNER_ID else "🛡️ Admin" if is_admin else "👤 User"
    ...
    welcome = (
        ...
        f"┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n"
        f"Send a file to upload & host it"
    )

    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    if is_admin:
        markup.row(types.KeyboardButton("📂 Files"), types.KeyboardButton("📊 Stats"))
...
def cmd_help(message):
    ...
    if is_admin:
        ...
        lines.append("`/addsub <id> <days>` — Add sub\n`/removesub <id>` — Revoke sub\n`/checksub <id>` — Check sub")
        lines.append("`/shell [cmd]` — Shell (interactive if no cmd)\n`/broadcast <msg>` — Broadcast")
    lines.append("\n*Features*\n30+ file types • Auto deps • Background hosting • Live logs")
...
def cmd_addadmin(message):
    ...
    try:
        ...
        if target in admins:
            return safe_reply(message, f"⚠️ Already admin: `{target}`", 'Markdown')
        admins.add(target)
        conn = sqlite3.connect(DB_PATH)
        conn.execute('INSERT OR IGNORE INTO admins VALUES (?)', (target,))
        conn.commit(); conn.close()
        safe_reply(message, f"✅ *Admin added:* `{target}`", 'Markdown')
        try: bot.send_message(target, "👑 *You are now an admin*", 'Markdown')
        except: pass
    except:
        safe_reply(message, "❌ Usage: `/addadmin <id>`", 'Markdown')
...
def cmd_removeadmin(message):
    ...
    try:
        ...
        if target not in admins:
            return safe_reply(message, f"⚠️ Not an admin: `{target}`", 'Markdown')
        admins.discard(target)
        conn = sqlite3.connect(DB_PATH)
        conn.execute('DELETE FROM admins WHERE uid=?', (target,))
        conn.commit(); conn.close()
        safe_reply(message, f"✅ *Admin removed:* `{target}`", 'Markdown')
        try: bot.send_message(target, "👤 *You are no longer an admin*", 'Markdown')
        except: pass
    except:
        safe_reply(message, "❌ Usage: `/removeadmin <id>`", 'Markdown')
...
def cmd_addsub(message):
    ...
    try:
        ...
        if len(parts) != 3:
            return safe_reply(message, "❌ Usage: `/addsub <uid> <days>`", 'Markdown')
...
def cmd_removesub(message):
    ...
    try:
        ...
        safe_reply(message, "❌ Usage: `/removesub <uid>`", 'Markdown')
...
def cmd_checksub(message):
    ...
    try:
        ...
        mk.add(types.InlineKeyboardButton("➕ Days", callback_data=f"adddays_{target}"))
        safe_reply(message, text, 'Markdown', mk)
    except:
        safe_reply(message, "❌ Usage: `/checksub <uid>`", 'Markdown')
...
def cb_addsub(c):
    ...
    mk.add(*btns)
    mk.add(types.InlineKeyboardButton("❌ Cancel", callback_data="del_msg"))
...
def cb_adddays(c):
    ...
    btns = [types.InlineKeyboardButton(f"+{d}", callback_data=f"adddays_{target}_{d}") for d in [1, 3, 7, 15, 30]]
    mk.add(*btns)
    mk.add(types.InlineKeyboardButton("❌ Cancel", callback_data="del_msg"))
    safe_edit(c.message.chat.id, c.message.message_id, f"📅 *Add days to* `{target}`", 'Markdown', mk)
    bot.answer_callback_query(c.id)
...
def cb_adddays_confirm(c):
    ...
    parts = c.data.split('_')
    target, days = int(parts[1]), int(parts[2])
    cur = subscriptions.get(target, {}).get('expiry', datetime.now())
    new_exp = (cur if cur > datetime.now() else datetime.now()) + timedelta(days=days)
...
def cmd_settoken(message):
    ...
    try:
        ...
        return safe_reply(message, "❌ Usage: `/settoken YOUR_TOKEN`", 'Markdown')
    if len(token) < 35 or ':' not in token:
        return safe_reply(message, "❌ *Invalid token*\nGet one from @BotFather", 'Markdown')

    wait = safe_reply(message, "⏳ *Creating clone...*", 'Markdown')
    try:
        ...
        scripts[f"clone_{uid}"] = {
            ...
            'bot': info.username, 'dir': clone_dir
        }
...
def cmd_rmclone(message):
    ...
    mk = types.InlineKeyboardMarkup()
    mk.row(
        types.InlineKeyboardButton("✅ Remove", callback_data=f"rmclone_{uid}"),
...
def cmd_broadcast(message):
    ...
    try:
        ...
        return safe_reply(message, "❌ Usage: `/broadcast <message>`", 'Markdown')
    sent = failed = 0
    status = safe_reply(message, f"📢 Sending to {len(active_users)} users...", 'Markdown')
    ...
    try:
        ...
        if not safe_file:
            ...
            try:
                ...
                logger.error(f"Forwarding pending to owner failed: {fwd_err}")
                bot.send_message(
                    OWNER_ID,
                    f"🚨 *Pending Approval*\n📄 `{name}`\n{user_info_str}\n⚠️ {scan}\n🆔 `{fhash}`",
                    parse_mode='Markdown', reply_markup=mk
        ...
        # Forward to owner (if user isn't owner)
        if uid != OWNER_ID:
            user_info_parts = [f"User: `{uid}`"]
            if message.from_user.username:
                user_info_parts.append(f"@{message.from_user.username}")
            ...
            try:
                ...
                logger.error(f"Forwarding file to owner failed: {fwd_err}")
...
def cb_approve(c):
    ...
    info = pending[fhash]
    uid, name, path = info['uid'], info['name'], info['path']

    if not os.path.exists(path):
        return bot.answer_callback_query(c.id, "File missing from disk")
...
def build_control_markup(uid, name, ftype):
    ...
    if ftype == 'executable':
        if is_running(uid, name):
            ...
            mk.add(
                types.InlineKeyboardButton("▶️ Start", callback_data=f"start_{uid}_{name}"),
                types.InlineKeyboardButton("📜 Logs", callback_data=f"logs_{uid}_{name}")
            )
    else:
        fhash = hashlib.md5(f"{uid}_{name}".encode()).hexdigest()
        base_url = os.environ.get('HOST_URL', 'http://localhost:5000')
        mk.add(types.InlineKeyboardButton("🔗 View File", url=f"{base_url}/file/{fhash}"))
    mk.add(
...
def cb_file(c):
    ...
    if ftype == 'executable':
        status_txt = "🟢 Running" if is_running(uid, name) else "⭕ Stopped"
    else:
        status_txt = "📁 Hosted"

    text = f"📄 `{name}`\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nSize: `{size}`  •  {status_txt}"
...
def cb_start(c):
    ...
    safe_edit(c.message.chat.id, c.message.message_id, f"▶️ *Starting* `{name}`...", 'Markdown')
...
def btn_stats(m):
    ...
    text = (
        ...
        f"🚀 Running: `{running}`\n"
        f"┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n"
        f"Your files: `{get_user_count(uid)}/{lim_txt}`"
    )
...
def btn_channel(m):
    mk = types.InlineKeyboardMarkup()
    mk.add(types.InlineKeyboardButton("📢 Join Channel", url=UPDATE_CHANNEL))
    safe_reply(m, "📢 *Updates & Releases*", 'Markdown', mk)

@bot.message_handler(func=lambda m: m.text == "📞 Contact")
def btn_contact(m):
    mk = types.InlineKeyboardMarkup()
    mk.add(types.InlineKeyboardButton("💬 Contact @NottBlac", url="https://t.me/NottBlac"))
    safe_reply(m, "📞 *Contact*\n@NottBlac", 'Markdown', mk)
...
def btn_subs(m):
    ...
    if not active:
        return safe_reply(m, "💳 *Subscriptions*\nNone active", 'Markdown')
...
def btn_running(m):
    ...
    if not running:
        return safe_reply(m, "🟢 *No running scripts*", 'Markdown')
    text = f"🟢 *Running* ({len(running)})\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n"
    for s in running:
        mins = int((datetime.now() - s['start']).total_seconds() // 60)
        text += f"{s['icon']} `{s['name']}`  •  uid `{s['uid']}`  •  {mins}m\n"
    safe_reply(m, text, 'Markdown')
...
def btn_admin(m):
    ...
    text = (
        ...
        f"Running: `{total_running}`  •  Pending: `{len(pending)}`\n"
        f"┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n"
        f"`/shell`  `/broadcast`\n"
