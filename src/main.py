...
# Startup cache cleanup with error handling
...
try:
    for pycache in glob.glob(os.path.join(os.path.dirname(__file__), '**', '__pycache__'), recursive=True):
        try:
            ...
            pass
    for pyc in glob.glob(os.path.join(os.path.dirname(__file__), '**', '*.pyc'), recursive=True):
        try:
            os.remove(pyc)
        except:
            pass
...
def safe_makedirs(path):
    """Create directories safely, handle read-only filesystems"""
    try:
...
def ensure_codeblock(text):
    """Ensure text is wrapped in codeblock"""
    if not text:
        return "```\n```"
    text = str(text).strip()
...
def format_error_log(error_text, max_chars=3500):
    """Format error logs - keep latest lines"""
    if not error_text:
...
def safe_send(chat_id, message, parse_mode='Markdown'):
    """Safe message sending with codeblock wrapping"""
    try:
        message = ensure_codeblock(message)
...
def safe_send_error(chat_id, error_text, title="⚠️ Error"):
    """Send formatted error messages"""
    try:
        formatted = format_error_log(error_text, max_chars=3500)
        message = f"{title}\n\n{ensure_codeblock(formatted)}"
...
def install_module(module_name, pkg_manager='pip'):
    """Install Python/Node modules safely with venv support"""
    ...
    try:
        if pkg_manager == 'pip':
            # Use --break-system-packages for venv compatibility
            cmd = [sys.executable, '-m', 'pip', 'install', '--break-system-packages', module_name]
        elif pkg_manager == 'apt':
            cmd = ['apt-get', 'install', '-y', module_name]
...
OWNER_USERNAME = os.getenv('OWNER_USERNAME', '@blcqt')
...
def get_public_ip():
    """Auto-detect public IP address of VPS"""
    try:
        import requests
...
def detect_host_url():
    # Check if manually set in .env (takes priority)
    if os.environ.get('HOST_URL'):
        return os.environ.get('HOST_URL', '').rstrip('/')
    
    # Auto-detect from platform environment variables
    ...
    # Auto-detect VPS public IP if AUTO_IP=true
    if os.environ.get('AUTO_IP', 'true').lower() == 'true':
        port = os.environ.get('PORT', '5000')
        public_ip = get_public_ip()
        if public_ip:
            print(f"🔍 Auto-detected VPS IP: {public_ip}")
...
def home():
    return (f"<html><head><title>HostingBot</title></head>"
            ...
            f"<h1>HostingBot</h1><p>by <b>{OWNER_USERNAME}</b> — Running</p></body></html>")
...
# Menu buttons - used in handlers
MAIN_MENU_BUTTONS = [
    '📁 Files', '👤 Profile', '📊 Stats', '❓ Help', 
    '👤 Owner', '📞 Contact', '🔧 Module', '🤖 Clone',
    '🔧 Env Vars', '🌐 Github', '🟢 Running', '📁 Subs',
    '⏳ Pending', '🤖 Clones', '👑 Admin', '🔒 Lock',
    '📁 All Files', '📁 Bot Logs'
]
...
def safe_send(chat_id, text, parse=None, markup=None, _retry=0):
    try: return bot.send_message(chat_id, text, parse_mode=parse, reply_markup=markup)
        ...
        if "Too Many Requests" in str(e) and _retry < 3: time.sleep(2 ** _retry); return safe_send(chat_id, text, parse, markup, _retry + 1)
        raise
...
def _do_execute(uid, file_path, msg, work_dir, name, ext, key, zip_name=None):
    ...
    try:
        ...
        # Note: shell_procs removed, using USE_DOCKER directly
        if USE_DOCKER:
            try:
        ...
        if use_docker_exec:
            # Docker is available but we use system-user isolation (not container isolation)
            # Files stay in system directories, not in Docker containers
            full_cmd = cmd
            preexec = None  # No preexec when using docker (docker handles it)
        else:
            # System-user isolation
            full_cmd = cmd
            preexec = resource_limits(uid)  # Apply resource limits for system user

        for attempt in range(1, 11):
            if attempt > 1 and msg: safe_edit(msg.chat.id, msg.message_id, f"{icon} *{lang}* — `{display_name}`\n🔄 Retry {attempt}...", 'Markdown')
            try:
                res = subprocess.run(full_cmd, capture_output=True, text=True,
                                     timeout=30, cwd=cwd, env=env, preexec_fn=preexec if not use_docker_exec else None)
                ...
                with open(log_path, 'w') as lf, open(stderr_path, 'w') as ef:
                    p = subprocess.Popen(full_cmd, stdout=lf, stderr=ef, cwd=cwd, env=env, preexec_fn=preexec if not use_docker_exec else None)
...
@bot.message_handler(func=lambda m: m.text == "🔧 Module")
def btn_module(msg):
    if not require_join(msg): return
    uid = msg.from_user.id
    ...
    waiting_env[uid] = {'mode': 'module'}

@bot.message_handler(func=lambda m: m.from_user and m.from_user.id in waiting_env and waiting_env[m.from_user.id].get('mode') == 'module' and m.text)
def handle_module_input(m):
    uid = m.from_user.id
    text = m.text.strip()
    ...
    if text in MAIN_MENU_BUTTONS or text.startswith('/'):
    ...
    msg = safe_reply(m, "⏳ Installing...", 'Markdown')
    output = install_module(module, pkg_mgr)
    
    result_text = f"✅ *Installation Output*\n\n{output}"
    
    try:
        if msg:
            safe_edit(m.chat.id, msg.message_id, result_text, 'Markdown')
    except:
        safe_reply(m, result_text, 'Markdown')
...
def build_main_keyboard(uid):
    ...
    # Public buttons - visible to everyone
    ...
    mk.row(types.KeyboardButton("🔧 Module"), types.KeyboardButton("🤖 Clone"))
    ...
    # Owner-only buttons - HIDDEN from everyone except owner
    if is_owner:
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*50}")
    print(f"  HostingBot — by {OWNER_USERNAME}")
