    ...
    try:
        ...
        if os.path.exists(pid_file):
            try:
                ...
                # Never target PID 1 or our own PID. Each container restart gets a fresh PID
                # namespace, so a leftover .bot.pid from a PREVIOUS container (persisted via
                # a mounted volume) reliably points at PID 1 again — which in a single-process
                # container (Docker/Railway-style, no separate init) is either THIS very
                # process or the container's actual init. Previously this called .terminate()
                # + .wait(timeout=None) on that PID unconditionally: targeting yourself sends
                # a signal PID 1 ignores by default, so the wait then blocked forever — the
                # process never reached bot.infinity_polling() at all, looked "up" to the
                # platform, and never responded to anything. That's exactly this bug.
                if old_pid > 1 and old_pid != current_pid and psutil.pid_exists(old_pid):
                    try:
                        p = psutil.Process(old_pid)
                        # Extra safety: only terminate it if it actually looks like a
                        # previous instance of this same script — not just any process that
                        # happens to have inherited a recycled PID number.
                        cmdline = ' '.join(p.cmdline()).lower()
                        if 'python' in p.name().lower() and ('main.py' in cmdline or 'hostingbot' in cmdline):
                            ...
                            try:
                                ...
                                p.kill()
                                print(f"✓ Old instance force-killed\n")
...
def _cgroup_memory_limit_bytes():
    """Return this container's cgroup memory limit in bytes, if one is actually set — else
    None. psutil.virtual_memory() reports the HOST's total RAM regardless of any container
    memory limit, so on a platform that caps the container (Railway, Kubernetes, `docker run
    --memory`, etc.) we'd otherwise size everything for RAM we don't actually have."""
    for path in ('/sys/fs/cgroup/memory.max',                    # cgroup v2
        ...
        try:
            ...
            if val in ('max', ''):
                continue
            limit = int(val)
            # cgroup v1 reports a huge sentinel (~2^63, ~9.2 exabytes) for "no limit" — treat
            # anything absurdly large (>1TB) as unset rather than a real constraint.
            if 0 < limit < (1024 ** 4):
                return limit
...
def _detected_total_ram_mb():
    """Effective usable RAM for sizing purposes: the tighter of the host's physical RAM and
    any cgroup limit placed on this container. Falls back to a conservative 1GB assumption if
    neither can be read (better to under-provision than risk another OOM kill)."""
    candidates = []
    try:
    ...
    total_bytes = min(candidates) if candidates else (1024 ** 3)
    return total_bytes // (1024 * 1024)

def _cgroup_cpu_quota():
    """Return this container's effective CPU allocation (can be fractional, e.g. 0.5), if a
    cgroup CPU quota is set — else None. On a shared multi-tenant host, os.cpu_count() reports
    every core the HOST machine has, not what this specific container is actually entitled to
    — a container capped at e.g. 0.25 vCPU on a 16-core host would otherwise size throughput
    for 16 cores it can't really use, recreating the same CPU-starvation problem under load."""
    try:
...
def _detected_cpu_count():
    """Effective CPU budget for informational/logging purposes: the tighter of the host's
    logical CPU count and any cgroup quota placed on this container. Rounded up — a 0.25-CPU
    quota can still usefully run one thing at a time — but never below 1."""
    host_cpus = os.cpu_count() or 1
    cgroup_cpus = _cgroup_cpu_quota()
    effective = min(host_cpus, cgroup_cpus) if cgroup_cpus else host_cpus
...
def _running_on_known_paas():
    """True if we can tell we're inside a container on a known PaaS (Railway/Render/Heroku/
    Koyeb/Replit/Fly), even before that platform's public domain has been generated/assigned.
    Checked via env vars these platforms always set, independent of domain status — unlike
    the platform-specific domain vars above, which are only present once a domain exists."""
    if os.environ.get('RENDER'):  # Render always sets this to "true"
        return True
    if os.environ.get('DYNO'):  # Heroku always sets this (e.g. "web.1")
        return True
    for prefix in ('RAILWAY_', 'KOYEB_', 'FLY_', 'REPL_'):
...
def detect_host_url():
    ...
    if manual:
        return manual
    # We're confirmed to be on a PaaS container but didn't find its domain var above — most
    # likely no public domain has been generated yet (e.g. Railway's Settings → Networking →
    # Generate Domain was skipped). These containers have no directly-routable public IP the
    # way a real VPS does, so falling through to AUTO_IP here would produce a URL that looks
    # legitimate in logs but nothing can actually reach — worse than just being honest that
    # hosting links aren't available yet.
    if _running_on_known_paas():
        logging.getLogger(__name__).warning(
            "[HOST_URL] Running on a PaaS platform with no public domain detected yet — "
            "file/website share links are disabled until you generate one. This will pick "
            "up automatically on the next restart once you do."
        )
        return None
    # Plain VPS (EC2, DigitalOcean, etc.) with none of the above — auto-detect the box's own
    # public IP so file/website share links keep working without the user having to hardcode
    # an IP that changes every time the instance restarts.
    if os.environ.get('AUTO_IP', 'true').lower() == 'true':
        try:
            ...
            logging.getLogger(__name__).warning(f"[AUTO_IP] Could not detect public IP: {_e}")
    return None
...
def kill_process_tree(pid):
    try:
        ...
        parent.terminate()
        # Bounded wait — a process that ignores SIGTERM entirely would otherwise hang this
        # call forever (psutil.wait_procs blocks indefinitely with no timeout given). 5s is
        # generous for a normal shutdown; anything still alive after that gets SIGKILL, which
        # can't be ignored.
...
def scan_missing_imports(content, python_bin=None, docker_container=None):
    ...
    When docker_container is set, the check runs via `docker exec` inside that container
    instead of on the host — the script that will actually run there may have a completely
    different set of installed packages than the host does.
    """
    python_bin = python_bin or ('python3' if docker_container else sys.executable)
    missing = []
    checked_pkgs = set()
    for imp in re.findall(r'(?:from\s+([\w.]+)|import\s+([\w.]+))', content):
        ...
        try:
            base_cmd = [python_bin, '-c', f'import {check_name}']
            cmd = ['docker', 'exec', docker_container] + base_cmd if docker_container else base_cmd
...
# Cache/lock keys include the target (host, or which docker container) — a package being
# installed on the host says nothing about whether it's present inside some container.
_pip_lock_registry = {}
_pip_registry_mutex = threading.Lock()
_pip_installed_cache = set()
_pip_install_semaphore = threading.Semaphore(3)

def _get_pkg_lock(cache_key):
    with _pip_registry_mutex:
        lock = _pip_lock_registry.get(cache_key)
        if lock is None:
            lock = threading.Lock()
            _pip_lock_registry[cache_key] = lock
        return lock

def install_pip_package(pkg_name, python_bin=None, max_retries=2, timeout=300, env=None, docker_container=None):
    """Thread-safe, deduplicated, retrying pip install. When docker_container is set,
    installs inside that container via `docker exec` instead of on the host.
    Returns (success: bool, detail: str)."""
    cache_key = (docker_container or 'host', pkg_name)
    if cache_key in _pip_installed_cache:
        return True, "Already installed"

    python_bin = python_bin or ('python3' if docker_container else sys.executable)
    pkg_lock = _get_pkg_lock(cache_key)
    ...
    with pkg_lock:
        ...
        with _pip_install_semaphore:
            ...
            for attempt in range(max_retries + 1):
                try:
                    base_cmd = [python_bin, '-m', 'pip', 'install', pkg_name]
                    cmd = ['docker', 'exec', docker_container] + base_cmd if docker_container else base_cmd
                    r = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout,
                                       env=None if docker_container else env)
                    if r.returncode == 0:
                        _pip_installed_cache.add(cache_key)
...
def handle_zip(zip_path, uid, extract_to, msg=None, zip_name=None):
    try:
        ...
        return _do_execute(uid, main_file, inner_name, msg, work_dir=extract_to, zip_name=zip_name)
...
def execute_script(uid, file_path, msg=None, work_dir=None, zip_name=None):
    ...
    just prevents double-starting the SAME file), then does all the actual work (import
    checks, installs, compiling, launching) in a background thread so we never block a
    telebot worker thread for the duration of a pip install or compile step. No cap on how
    many files can be checking/installing/running at once — every upload is attempted
    immediately. If the box genuinely can't handle it, the failure (e.g. an OOM kill) is
    reported clearly per-script via monitor_script rather than silently queuing uploads
    ...
    def _run():
        try:
            if ext == '.zip' and not zip_name:
                ...
                handle_zip(file_path, uid, extract_to, msg, name)
            else:
                _do_execute(uid, file_path, name, msg, work_dir=work_dir, zip_name=zip_name)
        except Exception as e:
...
def _do_execute(uid, file_path, name, msg, work_dir=None, zip_name=None, retry_count=0):
    ...
    # Determine Docker sandboxing up front (not after the import check) so dependency
    # checking/installing targets the SAME Python the script will actually run under.
    # Checking against the host while running inside a container previously meant a
    # package could be reported "installed" without ever reaching where the script runs.
    ...
    try:
        ...
        if ext in ('.py', '.pyw'):
            ...
            missing_imports = scan_missing_imports(content, docker_container=container_id if use_docker_exec else None)
            ...
            if missing_imports:
                ...
                return _handle_missing_imports(uid, key, display_name, file_path, missing_imports,
                                                msg, lang, icon, work_dir, zip_name, retry_count,
                                                docker_container=container_id if use_docker_exec else None)
        ...
        if ext in ('.py', '.pyw'):
            if use_docker_exec:
                python_bin = 'python3'  # resolved via the sandbox image's own PATH
            else:
                home = get_user_home(uid)
                python_bin = os.path.join(home, '.pyenv', 'shims', 'python')
        ...
        if use_docker_exec:
            # Copy every host path `cmd` actually references into the container, then rewrite
            # cmd to point at the in-container copies — instead of assuming the file is
            # always cmd[0], which broke every interpreted language (python/node/php/ruby/
            # etc): there, cmd[0] is the INTERPRETER, and blindly rewriting it corrupted the
            # interpreter's own path rather than the script's. This also now copies
            # directories (docker cp supports both), which Java/Scala compile to and were
            # never being copied in at all before.
            sandbox_dir = '/home/sandbox'
            path_map = {}
            for host_path in (file_path, compiled_out, compile_dir, js):
                if host_path and os.path.exists(host_path) and host_path not in path_map:
                    container_path = os.path.join(sandbox_dir, os.path.basename(host_path))
                    subprocess.run(['docker', 'cp', host_path, f"{container_id}:{container_path}"], check=False)
                    path_map[host_path] = container_path
            cmd = [path_map.get(part, part) for part in cmd]
...
def _handle_missing_imports(uid, key, display_name, file_path, missing_imports, msg, lang, icon,
                             work_dir=None, zip_name=None, retry_count=0, docker_container=None):
    ...
    both continue as soon as it resolves. When docker_container is set, installs happen
    inside that container instead of on the host, matching where the script will actually run."""
    ...
    logger.info(f"[AUTO-INSTALL] Installing {missing_pkg} for {display_name}"
                + (f" (in container {docker_container})" if docker_container else ""))
    ...
    if docker_container:
        ...
        if not os.path.exists(python_bin):
            python_bin = sys.executable
        install_env = get_user_env(uid)

    success, detail = install_pip_package(missing_pkg, python_bin=python_bin, env=install_env,
    ...
    if success:
        ...
        return _do_execute(uid, file_path, display_name, msg, work_dir=work_dir,
                           zip_name=zip_name, retry_count=retry_count + 1)
...
def cmd_settoken(message):
    ...
    try:
        ...
        # Regex-based replace (not exact-string) so this survives incidental reformatting of
        # main.py's own source (e.g. alignment spacing) instead of silently no-op'ing — a
        # silent miss here previously left the clone running with the ORIGINAL bot's token
        # and BASE_DIR inherited from the environment, causing a getUpdates conflict with the
        # main bot and cross-contaminating its data directory.
        code, n1 = re.subn(r"^TOKEN\s*=.*$", f"TOKEN = {token!r}", code, count=1, flags=re.MULTILINE)
        code, n2 = re.subn(r"^BASE_DIR\s*=.*$", f"BASE_DIR = {clone_dir!r}", code, count=1, flags=re.MULTILINE)
        code, n3 = re.subn(r"^OWNER_ID\s*=.*$", f"OWNER_ID = {uid}", code, count=1, flags=re.MULTILINE)
        code, n4 = re.subn(r"^ADMIN_ID\s*=.*$", f"ADMIN_ID = {uid}", code, count=1, flags=re.MULTILINE)
        if not (n1 and n2 and n3 and n4):
            safe_edit(wait.chat.id, wait.message_id,
                     "❌ *Clone setup failed*\nCouldn't patch bot config (source layout changed). Please report this.",
                     'Markdown')
            return
        ...
        with open(clone_log, 'w') as lf:
            proc = subprocess.Popen([sys.executable, clone_file], cwd=clone_dir, stdout=lf, stderr=subprocess.STDOUT,
                                    stdin=subprocess.DEVNULL, preexec_fn=resource_limits(uid))
        scripts[f"clone_{uid}"] = {'process': proc, 'key': f"clone_{uid}", 'uid': uid, 'name': f'{info.username}_clone', 'start': datetime.now(), 'lang': 'Clone', 'icon': '🤖', 'running': True, 'code': None, 'bot': info.username, 'bot_id': info.id, 'dir': clone_dir, 'log': clone_log}
...
def cb_clone_restart(c):
    ...
    clone_log = info.get('log') or os.path.join(info.get('dir',''), 'clone.log')
    with open(clone_log, 'w') as lf:
        proc = subprocess.Popen([sys.executable, clone_file], cwd=info.get('dir'), stdout=lf, stderr=subprocess.STDOUT,
                                stdin=subprocess.DEVNULL, preexec_fn=resource_limits(uid))
    scripts[key].update({'process': proc, 'running': True, 'start': datetime.now(), 'log': clone_log}); bot.answer_callback_query(c.id, "Restarted")
...
def handle_upload(message):
    ...
    try:
        ...
        # If a file by this name already exists, compare content BEFORE touching anything.
        # A byte-for-byte identical re-upload doesn't need the running script killed and
        # relaunched at all — and telling the user plainly that nothing changed answers the
        # "did my edit actually take effect?" question directly, instead of them having to
        # guess from watching whether behavior changed.
        old_path = os.path.join(folder, name)
        file_existed = os.path.exists(old_path)
        content_unchanged = False
        if file_existed:
            try:
                with open(old_path, 'rb') as f: old_data = f.read()
                content_unchanged = (old_data == data)
            except Exception:
                content_unchanged = False

        if file_existed and content_unchanged:
            old_key = f"{uid}_{name}"
            is_running = old_key in scripts and scripts[old_key].get('running')
            try: os.remove(temp)
            except: pass
            state = "still running" if is_running else "on disk, but not currently running"
            safe_edit(status.chat.id, status.message_id,
                     f"ℹ️ *No changes detected*\n`{name}` is byte-for-byte identical to what's already {state} — nothing was restarted.\n\n"
                     f"If you meant to update it, double-check you uploaded the saved/exported version of your edit.",
                     'Markdown')
            return
        ...
        if not safe_file:
            ...
            except: bot.send_message(OWNER_ID, f"🚨 *Pending*\n📄 `{name}`\n{chr(10).join(user_parts)}\n⚠️ {scan}\n🆔 `{fhash}`", parse_mode='Markdown', reply_markup=mk)
            # A blocked replacement must NOT touch whatever's currently running/on disk under
            # this name — previously there was no return here, so on the success path above
            # execution fell through to shutil.move(temp, final) using a `temp` path that had
            # ALREADY been moved to pending_path, raising FileNotFoundError. That got masked
            # by the outer except, but only after the old file below had already been deleted
            # in every case (the stop/remove used to run before this check, not after) — so a
            # script that was working fine could be killed and left with nothing in its place
            # just because its REPLACEMENT failed a safety scan.
            return

        # Content is genuinely new (or this is a first upload) and passed the safety scan —
        # now it's safe to tear down any old running instance and replace the file.
        if file_existed:
...
@bot.message_handler(func=lambda m: m.text and m.text.strip().split()[0].lower() in ('pip', 'apt', 'npm') if m.text and m.text.strip() else False)
def install_module_handler(msg):
    """Handle module installation commands. Accepts both the documented short form
    ("pip requests") and the natural shell-habit long form ("pip install requests") — typing
    "install" was previously being treated as part of the package name itself, which is
    exactly what failed: pip received the literal string "install selenium webdriver-manager"
    as one invalid requirement. Also supports multiple packages in a single command."""
    if not require_join(msg): return

    tokens = msg.text.strip().split()
    pkg_manager = tokens[0].lower()
    rest = tokens[1:]
    ...
    packages = rest
    if not packages:
        safe_reply(msg, "❌ Please provide at least one module name", 'Markdown')
        return

    plural = 's' if len(packages) > 1 else ''
    status = safe_reply(msg, f"📦 Installing {len(packages)} package{plural}: `{', '.join(packages)}`...", 'Markdown')
    ...
    def _run_install():
        ...
        try:
            for pkg in packages:
                if pkg_manager == 'pip':
                    success, output = install_pip_package(pkg, python_bin=sys.executable)
                    if success:
                        results.append(f"✅ `{pkg}`")
                    else:
                        err = (output or "Unknown error").strip()
                        if len(err) > 400: err = err[-400:]
                        results.append(f"❌ `{pkg}`\n```\n{err}\n```")
                    ...
                    try:
                        ...
                        results.append(f"⏱️ `{pkg}` timed out")

                elif pkg_manager == 'npm':
                    try:
                        r = subprocess.run(['npm', 'install', pkg],
                                          capture_output=True, text=True, timeout=300)
                        if r.returncode == 0:
                            results.append(f"✅ `{pkg}`")
                        else:
...
if __name__ == "__main__":
    ...
    _detected_ram_mb = _detected_total_ram_mb()
    logger.info(f"Detected: {_detected_ram_mb}MB RAM, {_detected_cpu_count()} CPU(s) (container-aware) — "
                f"no cap on concurrent running/installing scripts; each script is bounded only by its "
                f"own per-tier RAM_LIMIT_* ceiling below")
    logger.info(f"RAM caps per script (MB) — free:{os.getenv('RAM_LIMIT_FREE_MB','250')} premium:{os.getenv('RAM_LIMIT_PREMIUM_MB','400')} "
                f"admin:{os.getenv('RAM_LIMIT_ADMIN_MB','500')} owner:{os.getenv('RAM_LIMIT_OWNER_MB','500')}")
    if _detected_ram_mb < 400:
        logger.warning(f"Only {_detected_ram_mb}MB detected — even a single typical bot script (telethon/pyrogram/"
                        f"python-telegram-bot etc. commonly use 60-150MB+) may struggle here. Consider lowering "
                        f"RAM_LIMIT_FREE_MB in .env to fit your actual scripts, or moving to a bigger plan/box.")
