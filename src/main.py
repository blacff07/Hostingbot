...
def _auto_max_running_scripts():
    """Auto-scale how many scripts can run at once to the box this is actually running on,
    instead of one fixed number that's wrong for everyone except whoever it was tuned for.
    Reserves headroom for the OS/Docker/this bot's own process, then divides what's left by
    the free-tier per-script RAM ceiling — the same number that's actually enforced via
    RLIMIT_AS at launch time, so the two stay consistent with each other. Also capped by CPU
    count, generously, since these are I/O-bound bot processes that oversubscribe fine."""
    total_mb = _detected_total_ram_mb()
    reserved_mb = 300  # OS + Docker daemon + this bot's own process
    per_script_mb = max((TIER_RAM['free'] or (250 * 1024 * 1024)) // (1024 * 1024), 1)
    available_mb = max(total_mb - reserved_mb, per_script_mb)
    by_ram = max(1, available_mb // per_script_mb)
    try:
        by_cpu = max(1, (os.cpu_count() or 1) * 4)
    except Exception:
        by_cpu = by_ram
    return max(1, min(by_ram, by_cpu))

def _auto_max_concurrent_executions():
    """Throughput cap for simultaneous check/install/compile work. Lighter-weight and
    shorter-lived than a running script, so this can run a bit ahead of MAX_RUNNING_SCRIPTS
    without adding real memory pressure — mainly bounded by CPU here."""
    try:
...
def detect_host_url():
    ...
    if manual:
        return manual
    # Plain VPS (EC2, DigitalOcean, etc.) with none of the above platform env vars set:
    # auto-detect the box's own public IP so file/website share links keep working without
    # the user having to hardcode an IP that changes every time the instance restarts.
...
# extras wait their turn instead of piling on all together. Auto-scaled to this host's CPU count
# by default; set MAX_CONCURRENT_EXECUTIONS explicitly to override.
MAX_CONCURRENT_EXECUTIONS = int(os.getenv('MAX_CONCURRENT_EXECUTIONS') or _auto_max_concurrent_executions())
...
# _do_execute, released in monitor_script once the process actually exits. Auto-scaled to this
# host/container's detected RAM and CPU by default; set MAX_RUNNING_SCRIPTS explicitly to
# override (e.g. if you know your actual per-script usage differs from the RAM_LIMIT_* ceiling).
MAX_RUNNING_SCRIPTS = int(os.getenv('MAX_RUNNING_SCRIPTS') or _auto_max_running_scripts())
...
if __name__ == "__main__":
    ...
    logger.info(f"Detected: {_detected_total_ram_mb()}MB RAM, {os.cpu_count() or '?'} CPU(s) — "
                f"auto-scaled to Max running: {MAX_RUNNING_SCRIPTS} | Max checking/installing: {MAX_CONCURRENT_EXECUTIONS} "
                f"(override via MAX_RUNNING_SCRIPTS / MAX_CONCURRENT_EXECUTIONS in .env)")
