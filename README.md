...
### Method 4: Railway (PaaS)

Railway builds straight from the included `Dockerfile` (via `railway.toml`) — no VPS setup needed.

**1. Push this project to a GitHub repo, then in Railway:**
- New Project → Deploy from GitHub repo → select it

**2. Set environment variables** (Railway service → Variables tab) — same required vars as `.env.example`:
```
TELEGRAM_BOT_TOKEN=...
OWNER_ID=...
ADMIN_ID=...
OWNER_USERNAME=...
```
Don't set `PORT` yourself — Railway assigns it automatically and the bot reads whatever Railway provides. Leave `HOST_URL` unset too; Railway's own domain is detected automatically once you generate one (Settings → Networking → Generate Domain).

**3. Add a persistent volume** (Settings → Volumes) mounted at `/app/data` — without this, the bot's database and uploaded scripts are wiped on every redeploy, since Railway's container filesystem is otherwise ephemeral.

**4. Deploy.** Check the build logs for the same startup banner you'd see anywhere else (owner ID, detected RAM/CPU, auto-scaled limits).

Note: `USE_DOCKER` has no effect on Railway — there's no Docker socket available inside a Railway container, so script execution always uses system-user isolation there, same as any VPS without Docker access.
